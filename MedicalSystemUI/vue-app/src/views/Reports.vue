    ...
    <div class="analysis-section">
      ...
      <div class="gender-stats">
        <div class="gender-item">
          ...
          <span class="gender-count">{{ stats.maleCount }} ({{ stats.malePercentage }}%)</span>
        </div>
        <div class="gender-item">
          <span class="gender-label">Ženski:</span>
          <span class="gender-count">{{ stats.femaleCount }} ({{ stats.femalePercentage }}%)</span>
        </div>
      </div>
    </div>
    ...
    <div class="export-section">
      ...
      <div class="export-grid">
        <div class="export-card">
          <h3>📋 Pacijenti</h3>
          <p>Export svih pacijenata s osnovnim podacima</p>
          <button @click="exportPatients" :disabled="exporting.patients" class="btn btn-primary">
            {{ exporting.patients ? 'Exportiranje...' : 'Eksportiraj CSV' }}
          </button>
        </div>
        
        <div class="export-card">
          <h3>🏥 Pregledi</h3>
          <p>Export svih pregleda s detaljima</p>
          <button @click="exportExaminations" :disabled="exporting.examinations" class="btn btn-primary">
            {{ exporting.examinations ? 'Exportiranje...' : 'Eksportiraj CSV' }}
          </button>
        </div>
        
        <div class="export-card">
          <h3>💊 Recepti</h3>
          <p>Export svih recepta s lijekovima</p>
          <button @click="exportPrescriptions" :disabled="exporting.prescriptions" class="btn btn-primary">
            {{ exporting.prescriptions ? 'Exportiranje...' : 'Eksportiraj CSV' }}
          </button>
        </div>
        
        <div class="export-card">
          <h3>📋 Povijest bolesti</h3>
          <p>Export povijesti bolesti za sve pacijente</p>
          <button @click="exportMedicalHistory" :disabled="exporting.history" class="btn btn-primary">
            {{ exporting.history ? 'Exportiranje...' : 'Eksportiraj CSV' }}
          </button>
        </div>
      </div>
    </div>
  ...
  setup() {
    ...
    const loadStatistics = async () => {
      try {
        ...
        stats.value.malePercentage = total > 0 ? Math.round((maleCount / total) * 100) : 0
        stats.value.femalePercentage = total > 0 ? Math.round((femaleCount / total) * 100) : 0
    ...
    const exportPrescriptions = async () => {
      try {
        exporting.value.prescriptions = true
        await exportService.exportPrescriptions()
      } catch (err) {
        console.error('Export error:', err)
        alert('Greška pri exportu recepta')
      } finally {
        exporting.value.prescriptions = false
      }
    }
    
    const exportMedicalHistory = async () => {
      try {
        exporting.value.history = true
        await exportService.exportMedicalHistory()
      } catch (err) {
        console.error('Export error:', err)
        alert('Greška pri exportu povijesti bolesti')
      } finally {
        exporting.value.history = false
      }
    }
